...
namespace DM.Web.SL.Common.Utility.Data
{
    ...
    public abstract class DataFilter
    {
        ...
        {
            string sqlKeys = "exec↓select↓drop↓alter↓exists↓union↓and↓or↓xor↓order↓mid↓asc↓execute↓xp_cmdshell↓insert↓update↓delete↓join↓declare↓char↓sp_oacreate↓wscript.shell↓xp_regwrite↓'↓;↓--";
            string[] sqlArrs = sqlKeys.Split('↓');
            foreach (string sql in sqlArrs)
            {
                if (sqlstr.ToLower().Contains(sql))
                {
                    return true;
                }
            }
        ...
        public static string SqlFilter(string sqlstr)
        {
            string sqlKeys = "exec↓select↓drop↓alter↓exists↓union↓and↓or↓xor↓order↓mid↓asc↓execute↓xp_cmdshell↓insert↓update↓delete↓join↓declare↓char↓sp_oacreate↓wscript.shell↓xp_regwrite↓'↓;↓--";
            string[] sqlArrs = sqlKeys.Split('↓');
            foreach (string sqlkey in sqlArrs)
            {
