...
namespace DM.Web.SL.Common.Utility.Encrypt
{
    ...
    /// <summary>
    /// Summary description for MD5Provider.
    /// </summary>
    public class MD5Provider : IDisposable
    {
        #region Fields
        ...
        /// </summary>
        readonly uint[] _count = new uint[2];

        /// <summary>
        /// stat (ABCD)
        /// </summary>
        readonly uint[] _stat = new uint[4];

        #endregion Fields

        #region Constructors

        internal MD5Provider()
        {
            Initialize();
        }
        ...
        public static MD5Provider Create(string hashName)
        {
            if (hashName == "MD5Provider")
                return new MD5Provider();
            throw new NotSupportedException();
        }

        public static MD5Provider Create()
        {
            return new MD5Provider();
        }
        ...
        /// <summary>
        /// MD5Provider initialization. Begins an MD5Provider operation, writing a new context.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// MD5Provider block update operation. Continues an MD5Provider message-digest
        ...
        private void HashCore(byte[] input, int offset, int count)
        {
            ...
            if ((_count[0] += (uint)count << 3) < ((uint)count << 3))
                _count[1]++;
            ...
            if (count >= partLen)
            {
                Buffer.BlockCopy(input, offset, _buffer, index, partLen);
                Transform(_buffer, 0);
            ...
            Buffer.BlockCopy(input, offset + i, _buffer, index, count - i);
        }

        /// <summary>
        /// MD5Provider finalization. Ends an MD5Provider message-digest operation, writing the
        ...
        private byte[] HashFinal()
        {
            ...
            // Store stat in digest
            Encode(digest, 0, _stat, 0, 16);
        ...
        private static void Decode(uint[] output, int outputOffset, byte[] input, int inputOffset, int count)
        {
            ...
            for (i = outputOffset, j = inputOffset; j < end; i++, j += 4)
                output[i] = input[j] | (((uint)input[j + 1]) << 8) | (((uint)input[j + 2]) << 16) | (((uint)input[j + 3]) <<
            24);
        }
        ...
        /// <summary>
        /// MD5Provider basic transformation. Transforms stat based on 64 bytes block.
        /// </summary>
        /// <param name="block"></param>
        /// <param name="offset"></param>
        private void Transform(byte[] block, int offset)
        {
            uint a = _stat[0], b = _stat[1], c = _stat[2], d = _stat[3];
            var x = new uint[16];
